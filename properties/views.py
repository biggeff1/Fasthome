...
def property_detail(request, property_id):
    prop = get_object_or_404(
        Property.objects.select_related('property_type').prefetch_related('photos', 'features'),
...
def _save_dynamic_details(prop, post):
    ...
    for number in range(1, _positive(post.get('living_room_count')) + 1):
        LivingRoom.objects.create(
            property=prop,
            number=number,
            sofa=post.get(f'living_{number}_sofa') == 'on',
            coffee_table=post.get(f'living_{number}_table') == 'on',
            television=post.get(f'living_{number}_tv') == 'on',
            curtains=post.get(f'living_{number}_curtains') == 'on',
            fan=post.get(f'living_{number}_fan') == 'on',
            air_conditioning=post.get(f'living_{number}_ac') == 'on',
        )
    ...
    for number in range(1, _positive(post.get('bathroom_count')) + 1):
        Bathroom.objects.create(
            property=prop,
            number=number,
            location_type=post.get(f'bathroom_{number}_location', 'INTERIOR'),
            access_type=post.get(f'bathroom_{number}_access', ''),
            hot_water=post.get(f'bathroom_{number}_hot') == 'on',
            shower=post.get(f'bathroom_{number}_shower') == 'on',
            bathtub=post.get(f'bathroom_{number}_bathtub') == 'on',
            sink=post.get(f'bathroom_{number}_sink') == 'on',
            mirror=post.get(f'bathroom_{number}_mirror') == 'on',
            storage=post.get(f'bathroom_{number}_storage') == 'on',
        )
    ...
    for number in range(1, _positive(post.get('toilet_count')) + 1):
        Toilet.objects.create(
            property=prop,
            number=number,
            location_type=post.get(f'toilet_{number}_location', 'INTERIOR'),
            access_type=post.get(f'toilet_{number}_access', ''),
            toilet_type=post.get(f'toilet_{number}_type', 'BOWL'),
        )
...
def _save_consents(publication, post):
    declaration, _ = PropertyDeclaration.objects.get_or_create(
        publication=publication,
        defaults={'relationship_to_property': ''},
    )
    ...
    declaration.accepted_at = timezone.now() if all([
        declaration.right_to_offer_confirmed,
        declaration.accuracy_confirmed,
        declaration.photos_authentic_confirmed,
        declaration.authorization_confirmed,
        declaration.acknowledged_responsibility,
    ]) else None
    declaration.save()

    consent, _ = CollaborationConsent.objects.get_or_create(
        publication=publication,
        defaults={'terms_version': 'v1'},
    )
    ...
    consent.accepted_at = timezone.now() if all([
        consent.verification_accepted,
        consent.presentation_accepted,
        consent.visits_accepted,
        consent.management_accepted,
        consent.collaboration_accepted,
    ]) else None
    consent.save()
...
def _validate_publication_ready(publication):
    ...
    required = [prop.province, prop.city_or_territory, prop.neighborhood]
    if not all(required):
...
def property_edit(request, property_id):
    prop = get_object_or_404(
        Property.objects.select_related('publication', 'property_type'),
        property_id=property_id,
        owner=request.user,
    )
    if request.method == 'POST':
