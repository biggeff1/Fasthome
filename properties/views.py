...
from .models import (Bathroom, Bedroom, CollaborationConsent, Kitchen, LivingRoom,
                     Property, PropertyDeclaration, PropertyFeature, PropertyPublication,
                     PropertyType, Toilet)
...
def _save_dynamic_details(prop, post):
    ...
    for number in range(1, _positive(post.get('bedroom_count')) + 1):
        Bedroom.objects.create(
            ...
            bed_type=post.get(f'bed_{number}_type', ''),
            mattress=post.get(f'bed_{number}_mattress') == 'on',
            wardrobe=post.get(f'bed_{number}_wardrobe') == 'on',
            bedside_table=post.get(f'bed_{number}_bedside') == 'on',
            desk=post.get(f'bed_{number}_desk') == 'on',
            chair=post.get(f'bed_{number}_chair') == 'on',
            curtains=post.get(f'bed_{number}_curtains') == 'on',
            mosquito_net=post.get(f'bed_{number}_mosquito') == 'on',
            fan=post.get(f'bed_{number}_fan') == 'on',
            air_conditioning=post.get(f'bed_{number}_ac') == 'on',
        )
    ...
    for key in ['courtyard', 'garden', 'terrace', 'balcony', 'veranda', 'parking', 'garage', 'water_network', 'borehole', 'cistern', 'well', 'electric_network', 'generator', 'solar', 'security_guard', 'fence', 'secure_door', 'cameras', 'alarm', 'secure_parking']:
        if post.get(key) == 'on':
            PropertyFeature.objects.create(property=prop, key=key, category='EXTERIOR' if key in {'courtyard','garden','terrace','balcony','veranda','parking','garage'} else 'INFRASTRUCTURE' if key in {'water_network','borehole','cistern','well','electric_network','generator','solar'} else 'SECURITY', value='true')
...
def property_edit(request, property_id):
    ...
    if request.method == 'POST':
        ...
        prop.floor = request.POST.get('floor', '').strip(); prop.ceiling_type = request.POST.get('ceiling_type', '').strip(); prop.floor_type = request.POST.get('floor_type', '').strip()
        prop.province = request.POST.get('province', '').strip(); prop.city_or_territory = request.POST.get('city_or_territory', '').strip(); prop.administrative_subdivision = request.POST.get('administrative_subdivision', '').strip(); prop.neighborhood = request.POST.get('neighborhood', '').strip(); prop.exact_address = request.POST.get('exact_address', '').strip(); prop.google_maps_url = request.POST.get('google_maps_url', '').strip(); prop.latitude = request.POST.get('latitude') or None; prop.longitude = request.POST.get('longitude') or None
        prop.monthly_rent = request.POST.get('monthly_rent') or None; prop.guarantee_amount = request.POST.get('guarantee_amount') or None; prop.max_occupants = max(1, _positive(request.POST.get('max_occupants'), prop.max_occupants))
        ...
        if request.POST.get('submit') == '1':
            prop.publication.status = 'SUBMITTED'; prop.publication.submitted_at = timezone.now(); prop.publication.save(update_fields=['status','submitted_at','updated_at']); prop.status = 'UNDER_REVIEW'; prop.save(update_fields=['status','updated_at']); messages.success(request, 'Publication soumise à Fasthome pour vérification.')
