...
(() => {
  ...
  async function compress(file) {
    ...
    try {
      ...
      if (!sourceWidth || !sourceHeight) throw new Error(`Image illisible : ${file.name}`);
  ...
  function ensureStatus(form) {
    ...
    if (!status) {
      ...
      status.className = 'alert'; status.hidden = true;
  ...
  function uploadFormData(form, formData, status, total) {
    return new Promise((resolve, reject) => {
      ...
      xhr.upload.addEventListener('progress', (event) => {
        if (!event.lengthComputable) { status.textContent = `Envoi de ${total} photo${total > 1 ? 's' : ''}…`; return; }
        const percent = Math.round((event.loaded / event.total) * 100);
        status.textContent = `Envoi des photos : ${percent}%`;
      });
      xhr.addEventListener('load', () => {
        if (xhr.status >= 200 && xhr.status < 400) { resolve(xhr.responseURL || window.location.href); return; }
        const text = xhr.responseText || '';
        document.open(); document.write(text); document.close(); resolve(null);
      });
    ...
    try {
      ...
      for (let start = 0; start < allFiles.length; start += CONCURRENCY) {
        ...
        const results = await Promise.all(batch.map(async ({ input, file }) => {
          ...
          completed += 1; status.textContent = `Optimisation des photos : ${completed}/${total}`;
          await sleep(); return { input, optimized };
        }));
      ...
      optimizedByInput.forEach((files, name) => files.forEach((file) => { formData.append(name, file, file.name); optimizedTotal += file.size; }));
      status.textContent = total ? `Envoi de ${total} photo${total > 1 ? 's' : ''} (${Math.ceil(optimizedTotal / 1_000_000)} Mo)…` : 'Enregistrement…';
  ...
  function installUpload(form) {
    ...
    new MutationObserver(() => normalizeInputs(form)).observe(form, { childList: true, subtree: true });
