...
class VerificationModerationTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email='kyc-user@example.com', password='A-secure-password-123',
            phone='+243900009101', last_name='KYC', first_name='User',
        )
        self.staff = User.objects.create_user(
            email='kyc-staff@example.com', password='A-secure-password-123',
            phone='+243900009102', last_name='KYC', first_name='Staff',
            is_staff=True, can_review_kyc=True,
        )
        self.other_reviewer = User.objects.create_user(
            email='kyc-other@example.com', password='A-secure-password-123',
            phone='+243900009103', last_name='KYC', first_name='Other',
            is_staff=True, can_review_kyc=True,
        )
        self.verification = IdentityVerification.objects.create(
            user=self.user, assigned_reviewer=self.staff, document_type='PASSPORT',
    ...
    def test_face_requires_selfie(self):
        self.verification.facial_photo = None
    ...
    def test_rejection_requires_reason_and_revokes_certification(self):
        ...
        self.client.post(reverse('office_verification_decision', args=[self.verification.pk]), {'action': 'verify_document'})
        self.client.post(reverse('office_verification_decision', args=[self.verification.pk]), {'action': 'verify_face'})
        self.user.refresh_from_db(); self.assertTrue(self.user.is_certified)
    ...
    def test_kyc_reviewer_cannot_list_unassigned_verification(self):
        self.client.force_login(self.other_reviewer)
        response = self.client.get(reverse('office_verifications'))
        self.assertEqual(response.status_code, 200)
        self.assertNotContains(response, self.verification.user.fasthome_id)

    def test_assigned_kyc_reviewer_can_download_document(self):
        self.client.force_login(self.staff)
        response = self.client.get(reverse('office_verification_document', args=[self.verification.pk]))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response['Content-Disposition'], 'attachment; filename="identity.pdf"')
