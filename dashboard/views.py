...
def favorites(request):
    items=Favorite.objects.filter(user=request.user).select_related('property','property__property_type').order_by('-created_at'); return render(request,'dashboard/favorites.html',{'items':items})
...
def toggle_favorite(request,property_id):
    ...
    favorite.delete() if not created else None; messages.success(request,'Logement retiré des favoris.' if not created else 'Logement ajouté aux favoris.'); return redirect(request.POST.get('next') or request.META.get('HTTP_REFERER') or 'home')
...
def office_accept_case(request,case_id):
    ...
    if request.method=='POST':
        lease=Lease.objects.create(rental_case=case,property=case.property,tenant=case.tenant,landlord=case.property.owner,monthly_rent=case.property.monthly_rent or 0,guarantee_amount=case.property.guarantee_amount,status='PENDING')
        Contract.objects.create(lease=lease,contract_type='TENANT',status='PENDING'); Contract.objects.create(lease=lease,contract_type='LANDLORD',status='PENDING'); InspectionReport.objects.create(lease=lease,property=lease.property,report_type='ENTRY',status='DRAFT')
        case.status='CONTRACTING'; case.save(update_fields=['status']); Notification.objects.create(recipient=case.tenant,level='ACTION',title='Contrats en préparation',message='Fasthome a accepté votre dossier. Les deux contrats de la location sont en préparation.',object_type='Lease',object_id=lease.lease_id); Notification.objects.create(recipient=lease.landlord,level='ACTION',title='Contrat bailleur en préparation',message='Fasthome prépare le contrat relatif à votre logement.',object_type='Lease',object_id=lease.lease_id); messages.success(request,f'Location {lease.lease_id} créée avec deux contrats et un PV.')
