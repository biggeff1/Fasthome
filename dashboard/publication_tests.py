...
class PublicationModerationTests(TestCase):
    def setUp(self):
        ...
        self.staff = User.objects.create_user(
            email='publication-staff@example.com',
            password='A-secure-password-123',
            phone='+243900009002',
            last_name='Staff',
            first_name='Publication',
        ...
        self.publication = PropertyPublication.objects.create(
            property=self.property,
            status='SUBMITTED',
        )
        PropertyDeclaration.objects.create(
            publication=self.publication,
            relationship_to_property='Propriétaire',
            right_to_offer_confirmed=True,
            accuracy_confirmed=True,
            photos_authentic_confirmed=True,
    ...
    def test_staff_can_publish_submission(self):
        ...
        response = self.client.post(
            reverse('office_publication_decision', args=[self.publication.publication_id]),
            {'action': 'approve'},
            follow=True,
        )
    ...
    def test_staff_can_request_correction_with_reason(self):
        ...
        response = self.client.post(
            reverse('office_publication_decision', args=[self.publication.publication_id]),
            {'action': 'correction', 'reason': 'Corriger la localisation et ajouter les informations manquantes.'},
            follow=True,
        )
        self.assertEqual(response.status_code, 200)
        self.publication.refresh_from_db()
